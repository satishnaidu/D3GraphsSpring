...
var chart = d3.select("#chart").append("svg") // append svg element inside
// #chart
.attr("width", width + (2 * margin.left) + margin.right) // set width
.attr("height", height + margin.top + margin.bottom); // set height
var xAxis = d3.svg.axis().scale(x).orient("bottom"); // orient bottom because
// x-axis will appear
// below the bars

var yAxis = d3.svg.axis().scale(y).orient("left");

var url = "http:localhost:8080/graphs/barchart";
// For actual integration, we will make backend rest call to get the data from
// the spring applicaiton.
d3.json("http://codepen.io/superpikar/pen/kcJDf.js", function(error, data) {
	...
	}).attr("width", x.rangeBand()); // set width base on range on ordinal
	// data

	bar.append("text").attr("x", x.rangeBand() + margin.left).attr("y",
			function(d) {
	...
	chart.append("g").attr("class", "x axis").attr("transform",
			"translate(" + margin.left + "," + height + ")").call(xAxis);
